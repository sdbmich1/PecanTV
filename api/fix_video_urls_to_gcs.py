#!/usr/bin/env python3
"""
Fix video URLs to use the correct GCS URLs that actually exist
"""
...
def fix_video_urls_to_gcs():
    """Fix video URLs to use correct GCS URLs"""
    
    with engine.connect() as conn:
        print("🔧 Fixing video URLs to use correct GCS URLs...")
        print("=" * 50)
        
        # 1. Fix Longstreet episodes - use the GCS URLs that actually exist
        ...
        for episode_id, episode_num, title, old_url in longstreet_episodes:
            # Use GCS URLs that actually exist
        ...
        for episode_id, episode_num, title, old_url in monte_cristo_episodes:
            # Use GCS URLs that actually exist
            gcs_url = f"https://storage.googleapis.com/pecantv_series/count_of_monte_cristo/Count-of-Monte-Cristo-Episode-{episode_num}.mp4"
        ...
        for episode_id, episode_num, title, old_url in man_with_camera_episodes:
            # Use GCS URLs that actually exist
            gcs_url = f"https://storage.googleapis.com/pecantv_series/man_with_a_camera/Man_with_a_Camera_Episode_{episode_num}.mp4"
        ...
        for episode_id, episode_num, title, old_url in ghost_squad_episodes:
            # Use GCS URLs that actually exist
            gcs_url = f"https://storage.googleapis.com/pecantv_series/ghost_squad/Ghost_Squad_Episode_{episode_num}.mp4"
        ...
        conn.commit()
        print(f"\n🎉 Successfully updated all video URLs to GCS!")

if __name__ == "__main__":
    fix_video_urls_to_gcs() 