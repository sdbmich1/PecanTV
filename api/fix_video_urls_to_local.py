#!/usr/bin/env python3
"""
Fix video URLs to use local paths instead of GCS URLs since the GCS files don't exist
"""
...
def fix_video_urls_to_local():
    ...
    with engine.connect() as conn:
        ...
        for episode_id, episode_num, title, old_url in longstreet_episodes:
            # Use local path instead of GCS
            local_url = f"http://localhost:8000/pecantv_series/longstreet/Longstreet{episode_num}.mp4"
        ...
        for episode_id, episode_num, title, old_url in monte_cristo_episodes:
            # Use local path instead of GCS
            local_url = f"http://localhost:8000/pecantv_series/the_count_of_monte_cristo/count_of_monte_cristo/Count-of-Monte-Cristo-Episode-{episode_num}.mp4"
        ...
        for episode_id, episode_num, title, old_url in man_with_camera_episodes:
            # Use local path instead of GCS
            local_url = f"http://localhost:8000/pecantv_series/man_with_a_camera/Man_with_a_Camera_Episode_{episode_num}.mp4"
        ...
        for episode_id, episode_num, title, old_url in ghost_squad_episodes:
            # Use local path instead of GCS
            local_url = f"http://localhost:8000/pecantv_series/ghost_squad/Ghost_Squad_Episode_{episode_num}.mp4"
        ...
        conn.commit()
        print(f"\n🎉 Successfully updated all video URLs to local paths!")

if __name__ == "__main__":
    fix_video_urls_to_local() 