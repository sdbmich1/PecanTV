...
def check_logo_files():
    ...
    # Check main logo file (correct path)
    ...
    if os.path.exists(contents_path):
        ...
        with open(contents_path, 'r') as f:
            print(f"   Configuration: {f.read().strip()}")
    else:
    ...
    if os.path.exists(launch_screen_path):
        ...
        with open(launch_screen_path, 'r') as f:
            ...
            if 'pecantv_logo' in content:
                ...
                print("   Current content shows it might be using AppIcon instead")
    else:
        print(f"❌ Launch screen missing: {launch_screen_path}")

def check_splash_screen():
    """Check splash screen code."""
    print("\n📱 Checking Splash Screen Code...")
    print("=" * 50)
    
    splash_screen_path = "PecanTV/PECANTV/PECANTV/Features/Authentication/Views/SplashScreenView.swift"
    if os.path.exists(splash_screen_path):
        print(f"✅ Splash screen code exists: {splash_screen_path}")
        
        # Check if it references the logo
        with open(splash_screen_path, 'r') as f:
            content = f.read()
            if 'pecantv_logo' in content:
                print("✅ Splash screen references pecantv_logo")
            else:
                print("❌ Splash screen doesn't reference pecantv_logo")
    else:
        print(f"❌ Splash screen code missing: {splash_screen_path}")
