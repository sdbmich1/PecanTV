#!/usr/bin/env python3
"""
Script to update Bonanza episodes with content URLs using the 'Video Filename' field 
and 'Artwork Filename' for poster URLs from the Bonanza files.
"""
...
def load_bonanza_data():
    """Load Bonanza episode data from all files."""
    bonanza_episodes = {}
            ...
            if 'Series Name' in df.columns:
                ...
                for _, row in bonanza_df.iterrows():
                    ...
                    if video_filename and video_filename != 'nan':
                        key = (season, episode)
                        if key not in bonanza_episodes:
                            bonanza_episodes[key] = {
                                'title': title,
...
def update_bonanza_episodes():
    """Update Bonanza episodes with content URLs and poster URLs."""
    print("🎬 Updating Bonanza Episodes with Content URLs")
    print("=" * 50)
    
    # Load Bonanza data
    bonanza_data = load_bonanza_data()
    ...
    try:
        ...
        for ep_id, ep_title, season, episode, current_content_url, current_poster_url in missing_episodes:
            key = (season, episode)
            bonanza_info = bonanza_data.get(key)
        ...
        cur.execute("""
            SELECT 
                COUNT(*) as total,
                COUNT(CASE WHEN content_url IS NULL OR content_url = '' THEN 1 END) as missing,
                COUNT(CASE WHEN content_url IS NOT NULL AND content_url != '' THEN 1 END) as valid
            FROM episodes e
