#!/usr/bin/env python3
"""
Script to check and fix Ghost Squad episodes using Video Filename and Artwork Filename from Wurl metadata.
"""
...
def check_and_fix_ghost_squad():
    """Check and fix Ghost Squad episodes."""
    print("🔧 Checking and fixing Ghost Squad episodes")
    print("=" * 60)
    ...
    # Filter for Ghost Squad episodes
    ghost_squad_data = df[df['Series Name'] == 'Ghost Squad']
    print(f"📺 Found {len(ghost_squad_data)} Ghost Squad episodes in Wurl data")
    
    if ghost_squad_data.empty:
        print("❌ No Ghost Squad data found in Wurl file")
        return
    ...
    try:
        ...
        for _, row in ghost_squad_data.iterrows():
            ...
            content_url = f"pecantv_series/ghost_squad/{video_filename}" if video_filename else ''
            poster_url = f"pecantv_series/ghost_squad/{artwork_filename}" if artwork_filename else ''
        ...
        conn.commit()
        print(f"\n🎉 Successfully updated {updated_count} Ghost Squad episodes!")
