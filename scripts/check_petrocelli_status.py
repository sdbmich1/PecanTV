#!/usr/bin/env python3
"""
Script to check the current status of Petrocelli episodes in the database.
"""
...
def check_petrocelli_status():
    """Check Petrocelli episodes status."""
    print("🔍 Checking Petrocelli episodes status")
    print("=" * 50)
    ...
    try:
        ...
        if not episodes:
            print("📺 No episodes found for Petrocelli")
            return
        ...
        print(f"Expected episodes: 22")
        
        if len(episodes) == 22:
            print("✅ Correct number of episodes!")
        elif len(episodes) > 22:
            print(f"⚠️  Too many episodes - need to delete {len(episodes) - 22}")
        else:
            print(f"⚠️  Too few episodes - need to add {22 - len(episodes)}")
