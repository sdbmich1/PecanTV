#!/usr/bin/env python3
"""
Script to check the current status of Ghost Squad episodes in the database.
"""
...
def check_ghost_squad_status():
    """Check Ghost Squad episodes status."""
    print("🔍 Checking Ghost Squad episodes status")
    print("=" * 50)
    ...
    try:
        ...
        print(f"✅ Found Ghost Squad series: {series_title} (ID: {series_id})")
        
        # Get episodes for Ghost Squad
        cur.execute("""
        ...
        if not episodes:
            print("📺 No episodes found for Ghost Squad")
            return
