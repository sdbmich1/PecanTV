#!/usr/bin/env python3
"""
Script to update Bonanza episodes 1-13 with video filenames from Wurl metadata.
"""
...
def load_wurl_data():
    """Load data from the Wurl metadata files."""
    # Try different files that contain Bonanza Animated Series data
    files_to_try = [
        "Wurl - File Upload Metadata_Version 7.0.12.xlsx",
    ...
    for file_path in files_to_try:
        try:
            ...
            if not bonanza_data.empty:
                print(f"✅ Found Bonanza Animated Series data in {file_path}")
                return bonanza_data
...
def update_bonanza_episodes():
    """Update Bonanza episodes 1-13 with video filenames from Wurl metadata."""
    print("🔧 Updating Bonanza episodes 1-13 with Wurl metadata")
    print("=" * 60)
    ...
    print(f"📺 Found {len(bonanza_data)} Bonanza Animated Series episodes in Wurl data")
    ...
    try:
        ...
        for _, row in bonanza_data.iterrows():
            ...
            # Only process episodes 1-13 (the ones missing URLs)
            if episode_number < 1 or episode_number > 13:
                continue
