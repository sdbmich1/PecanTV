#!/usr/bin/env python3
"""
Script to check the current status of Longstreet episodes in the database.
"""
...
def check_longstreet_status():
    """Check Longstreet episodes status."""
    print("🔍 Checking Longstreet episodes status")
    print("=" * 50)
    ...
    try:
        ...
        if not episodes:
            print("📺 No episodes found for Longstreet")
            return
        ...
        print(f"Expected episodes: 30")
        
        if len(episodes) == 30:
            print("✅ Correct number of episodes!")
        elif len(episodes) > 30:
            print(f"⚠️  Too many episodes - need to delete {len(episodes) - 30}")
        else:
            print(f"⚠️  Too few episodes - need to add {30 - len(episodes)}")
