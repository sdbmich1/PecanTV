#!/usr/bin/env python3
"""
Script to check the current status of Commando Cody episodes in the database.
"""
...
def check_commando_cody_status():
    """Check Commando Cody episodes status."""
    print("🔍 Checking Commando Cody episodes status")
    print("=" * 50)
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE title ILIKE 'Commando Cody%' AND type = 'SERIES'
        """)
        result = cur.fetchone()
        ...
        print(f"✅ Found Commando Cody series: {series_title} (ID: {series_id})")
        
        # Get episodes for Commando Cody
        cur.execute("""
        ...
        if not episodes:
            print("📺 No episodes found for Commando Cody")
            return
        
        print(f"📺 Found {len(episodes)} Commando Cody episodes:")
        print("=" * 50)
